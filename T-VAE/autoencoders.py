...
from model_outputs import BaseVAE_Output
...
class LatentEncoderNTokens(nn.Module):
    ...
    def forward(self, encoding) -> torch.Tensor:
        # only retain number of self.n_tokens tokens for laten space
...
class LatentDecoderNTokens(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.latent_size == config.t5.d_model:
            ...
            self.latent_to_token = nn.Linear(
                self.latent_size, config.t5.d_model)
...
class LatentDecoderFunnelNorm(LatentDecoderNTokens):
    ...
    def __init__(self, config):
        ...
        self.norm = nn.LayerNorm(
            config.funnel.d_model, config.funnel.layer_norm_eps)
...
class EncoderDecoderVAE(nn.Module):
    ...
    def forward(
        ...
        if input_encoding is None and latent is None:
            raise ValueError(
                "Both `input_encoding` and `latent` sent to VAE are None.")
        # don't regularise if given latent
        use_reg_loss = self.use_reg_loss and latent is None and skip_reg_loss is False
        recon_encoding, latent = self._model_forward(
            input_encoding, latent=latent)
        if use_reg_loss:
            ...
            reg_loss = self._regularliser_loss(
                latent.reshape(-1, latent_code_dim)) / batch_size * n_latents_per_batch
        else:
