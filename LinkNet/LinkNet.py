...
# basic residual block in encoder
class BasicBlock(nn.Module):

    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0,
                 groups=1, bias=False):
        super(BasicBlock, self).__init__()
        # conv1: in_planes -> out_planes
        self.conv1 = nn.Conv2d(
            in_planes, out_planes, kernel_size, stride, padding,
            groups=groups, bias=bias)
        self.bn1 = nn.BatchNorm2d(out_planes)
        self.relu = nn.ReLU(inplace=True)
        # conv2: out_planes -> out_planes
        self.conv2 = nn.Conv2d(
            out_planes, out_planes, kernel_size, 1, padding,
            groups=groups, bias=bias)
...
class LinkNet(nn.Module):
    ...
    def __init__(self, n_classes=21):
        ...
        base = resnet.resnet18(weights=ResNet18_Weights)
...
class LinkNetBase(nn.Module):
    ...
    def __init__(self, n_classes=21):
        ...
        # [conv(7 * 7), channels: (3, 64), stride: 2, padding: 3]
