...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # if downsample is not none, it indicates the dotted line in papaer.
...
class ResNet(torch.nn.Module):
    ...
    def __init__(self, block, layers, num_classes, zero_init_residual=False,
        ...
        self.conv1 = torch.nn.Conv2d(3, self.inplanes, kernel_size=7, stride=2,
    ...
    def _make_layer(self, block, planes, blocks,
        ...
        if dilate:
            ...
            stride = 1
        # detect differnce between input and output dimensions
        # and change downsample to create a mapping
        ...
        # it will automatically generate output for next conv

    def forward(self, x):
        x = self.conv1(x)  # Conv2d 7*7
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # 3*3, 2
